...
/** Backend uses cross-platform names; on macOS the UI shows Apple key glyphs only. */
function formatModifierDisplayName(name, isMacOS) {
    if (!isMacOS || !name) {
        return name;
    }
    let text = name;
    text = text.replaceAll("LeftWin", "Left ⌘");
    text = text.replaceAll("RightWin", "Right ⌘");
    text = text.replaceAll("LeftAlt", "Left ⌥");
    text = text.replaceAll("RightAlt", "Right ⌥");
    text = text.replaceAll("LeftControl", "Left ⌃");
    text = text.replaceAll("RightControl", "Right ⌃");
    return text;
}
...
function AddModifierMenuButton({ modifiers, onPick, isMacOS }) {
            ...
            >
                {available.map((modifier) => (
                    <MenuItem
                        ...
                        sx={{ fontSize: "13px" }}
                    >
                        {formatModifierDisplayName(modifier, isMacOS)}
                    </MenuItem>
                ))}
            </Menu>
        </>
                        ...
                        <Box sx={{ marginBottom: "24px" }}>
                            ...
                            >
                                {(group.modifiers || []).map((modifier) => (
                                    <Chip
                                        key={modifier}
                                        label={formatModifierDisplayName(
                                            modifier,
                                            isMacOS,
                                        )}
                                        onDelete={() =>
