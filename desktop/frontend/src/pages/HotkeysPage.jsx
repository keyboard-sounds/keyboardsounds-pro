...
import { Box, Typography, TextField, Select, MenuItem, IconButton, Button, Chip, FormControl, InputLabel, OutlinedInput } from '@mui/material';
...
function HotkeysPage() {
  ...
  const removeHotkey = (groupIndex, keyIndex) => {
    const updated = [...hotkeys];
    updated[groupIndex].keys = updated[groupIndex].keys.filter((_, index) => index !== keyIndex);
    setHotkeys(updated);
  };
  ...
  const updateHotkey = (groupIndex, keyIndex, field, value) => {
    ...
    if (field === 'key') {
      ...
      updated[groupIndex].keys[keyIndex].action = {
        ...updated[groupIndex].keys[keyIndex].action,
        [field === 'target' ? 'target' : field === 'action' ? 'action' : 'value']: value
      };
    }
    setHotkeys(updated);
  };
  ...
  const removeModifier = (groupIndex, modifier) => {
    const updated = [...hotkeys];
    updated[groupIndex].modifiers = (updated[groupIndex].modifiers || []).filter(m => m !== modifier);
    setHotkeys(updated);
  };
    ...
    return (
      <Box>
        ...
        <GlassCard sx={{ padding: '32px', textAlign: 'center' }}>
          <Typography sx={{ color: 'var(--text-secondary)' }}>Loading hotkeys...</Typography>
        </GlassCard>
      </Box>
    );
  }
    ...
    <Box>
      ...
      {error && (
        <GlassCard sx={{ 
          marginBottom: '24px', 
          padding: '16px 20px',
          background: 'linear-gradient(135deg, rgba(239, 68, 68, 0.12) 0%, var(--card-bg))',
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: '16px' }}>
          ...
          <Box sx={{ flex: 1 }}>
            ...
            >
              Hotkeys will only function when the application is enabled. Make sure the app is running and enabled for your configured hotkeys to work.
            </Typography>
          </Box>
        </Box>
      </GlassCard>
      ...
      {hotkeys.length === 0 ? (
        ...
        >
          ...
          >
            <KeyboardIcon sx={{ fontSize: '40px', color: 'var(--accent-primary)', opacity: 0.6 }} />
          </Box>
          <Typography 
            variant="h6" 
            sx={{ 
          ...
          >
            Create your first hotkey group to configure keyboard shortcuts for controlling the application.
          </Typography>
          <Button
            variant="contained"
            startIcon={<AddIcon />}
        ...
        hotkeys.map((group, groupIndex) => (
          <GlassCard key={groupIndex} sx={{ marginBottom: '24px', padding: '28px' }}>
            ...
            <Box sx={{ marginBottom: '24px' }}>
              ...
              <FormControl fullWidth size="small">
                <InputLabel sx={{ color: 'var(--text-secondary)' }}>Add Modifier</InputLabel>
                <Select
                  value=""
                  onChange={(e) => {
                    if (e.target.value) {
                ...
                >
                  {MODIFIER_OPTIONS.filter(m => !(group.modifiers || []).includes(m)).map((modifier) => (
                ...
                >
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
                    <FormControl size="small">
                      <InputLabel sx={{ color: 'var(--text-secondary)' }}>Key</InputLabel>
                      <Select
                        value={hotkey.key || ''}
                        onChange={(e) => updateHotkey(groupIndex, keyIndex, 'key', e.target.value)}
                    ...
                    <FormControl size="small">
                      <InputLabel sx={{ color: 'var(--text-secondary)' }}>Action</InputLabel>
                      <Select
                        value={hotkey.action?.action || 'mute'}
                        onChange={(e) => updateHotkey(groupIndex, keyIndex, 'action', e.target.value)}
                  ...
                  <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <FormControl size="small">
                      <InputLabel sx={{ color: 'var(--text-secondary)' }}>Device</InputLabel>
                      <Select
                        value={hotkey.action?.target || 'all'}
                        onChange={(e) => updateHotkey(groupIndex, keyIndex, 'target', e.target.value)}
                    ...
                    {(hotkey.action?.action === 'increase-volume' || hotkey.action?.action === 'decrease-volume') && (
                      <TextField
                        size="small"
                        label="Value"
                        type="number"
                        value={hotkey.action?.value || ''}
                        onChange={(e) => updateHotkey(groupIndex, keyIndex, 'value', e.target.value)}
