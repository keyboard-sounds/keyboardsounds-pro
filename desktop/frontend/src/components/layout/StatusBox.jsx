...
function StatusBox({
    ...
    const toggleBusy = isLoading || isStarting;
    ...
    const displayLastFocusedAppName =
        getMacOSAppBundleNameFromPath(lastFocusedApp?.appPath) ||
        lastFocusedApp?.executableName ||
        "";

    const [lastFocusedAppIcon, setLastFocusedAppIcon] = useState(null);
    useEffect(() => {
        let cancelled = false;

        const loadIcon = async () => {
            if (!lastFocusedApp?.appPath || platform !== "darwin") {
                setLastFocusedAppIcon(null);
                return;
            }
            try {
                const fn = window?.go?.app?.AppRules?.GetMacOSApplicationIcon;
                if (typeof fn !== "function") return;
                const dataUrl = await fn(lastFocusedApp.appPath);
                if (cancelled) return;
                setLastFocusedAppIcon(
                    typeof dataUrl === "string" && dataUrl ? dataUrl : null,
                );
            } catch {
                if (!cancelled) setLastFocusedAppIcon(null);
            }
        };
    ...
    }, [platform, lastFocusedApp?.appPath]);
    ...
    return (
        <Box
            sx={{
                margin: "16px 12px",
                marginTop: platform === "darwin" ? "4px" : "16px",
                padding: "16px",
                width: "calc(100% - 24px)",
                ...
                boxShadow:
                    "var(--card-shadow), inset 0 1px 0 var(--card-highlight)",
            }}
        >
            {/* Enable/Disable Toggle Button */}
            <Box
                onClick={() => !toggleBusy && setIsPaused(!isPaused)}
                sx={{
            ...
            {lastFocusedApp?.appPath && (
                ...
                >
                    ...
                    >
                        {platform === "darwin" && lastFocusedAppIcon ? (
                            <Box
                                component="img"
                                alt=""
                                src={lastFocusedAppIcon}
                                sx={{
                                    width: "16px",
                                    height: "16px",
                                    objectFit: "contain",
                    ...
                    >
                        {displayLastFocusedAppName}
                    </Typography>
                    <Button
                        size="small"
                        onClick={onAddRuleForLastFocusedApp}
                    ...
                    <IconButton
                        size="small"
                        onClick={onDismissLastFocusedApp}
                        sx={{
                            width: "22px",
                            height: "22px",
            ...
            <Box
                sx={{
                    marginBottom: !hideDefaultProfileSection ? "12px" : "0px",
            ...
            >
                ...
                >
                    ...
                    <Tooltip
                        title={
                            volumesLocked
                                ? "Volumes are linked - click to unlink"
                                : "Volumes are independent - click to link"
                        }
                        arrow
                        placement="top"
                    >
                        <IconButton
                            ...
                            sx={{
                                ...
                                color: volumesLocked
                                    ? "var(--accent-primary)"
                                    : "var(--text-tertiary)",
                                backgroundColor: volumesLocked
                                    ? "var(--accent-bg)"
                                    : "transparent",
                                border: volumesLocked
                    ...
                    >
                        ...
                        <FormControl fullWidth size="small" disabled={isPaused}>
                            <Select
                                ...
                                onChange={(e) =>
                                    setKeyboardProfile(e.target.value)
                                }
                                disabled={isPaused}
                                sx={{
                                    ...
                                    fontWeight: 500,
                                    boxShadow:
                                        "inset 0 1px 2px rgba(0, 0, 0, 0.1), 0 1px 0 var(--card-highlight)",
                    ...
                    >
                        ...
                        <FormControl fullWidth size="small" disabled={isPaused}>
                            <Select
                                value={mouseProfile}
                                onChange={(e) =>
                                    setMouseProfile(e.target.value)
                                }
                                disabled={isPaused}
                                sx={{
                                    backgroundColor: "var(--input-bg)",
