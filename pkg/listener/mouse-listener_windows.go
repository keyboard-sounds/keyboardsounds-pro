...
func NewMouseListener() MouseListener {
	...
	return &windowsMouseListener{
		events:     make(chan listenertypes.ButtonEvent, 100),
		workerDone: make(chan struct{}),
	}
}

type windowsMouseListener struct {
	...
	ringBuffer   [ringBufferSize]mouseEventData
...
func (l *windowsMouseListener) hook(ctx context.Context) hookFunc {
	return func(nCode int, wParam, lParam uintptr) uintptr {
		...
		if nCode >= 0 {
			// Parse the mouse message to get button and action
			button, action, ok := parseMouseMessage(uint32(wParam))
			if !ok {
				// Not a button press/release event we care about, skip it
				return r1
			}
			...
			l.ringBuffer[idx] = mouseEventData{
...
func (l *windowsMouseListener) eventWorker(ctx context.Context) {
	defer func() {
	...
	for {
		...
		if readPos < writePos {
			...
			eventData := l.ringBuffer[idx]

			// Advance read position
			atomic.StoreUint64(&l.ringReadPos, readPos+1)

			// Build the event
			event := listenertypes.ButtonEvent{
				Device:    &WindowsMouseDevice,
				Button:    eventData.Button,
				Action:    eventData.Action,
				Timestamp: parseTime(eventData.Time),
			}
...
func (l *windowsMouseListener) executeMessageLoop(ctx context.Context) {
	...
	for {
		...
		// For low-level mouse hooks, we don't need to translate/dispatch,
