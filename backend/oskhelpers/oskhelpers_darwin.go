...
func (d *darwinOSKHelper) SetOnScreenText(config OSKHelperConfig, text string) error {
	...
	text = strings.ReplaceAll(text, "LeftControl", "⌃")
	text = strings.ReplaceAll(text, "RightControl", "⌃")
	...
	// DismissAfter is applied only in ClearOnScreenText (same as Windows): the overlay stays
	// while the chord is active and hides after the delay once the application clears it.
	return nil
}
